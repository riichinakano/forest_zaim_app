...
2. **質問入力**
   ...
   - 質問例:
     ...
     - 「R6とR5の借入返済額を比較して」
     - 「過去5年間の役員報酬推移をグラフ化して」
     - 「令和6年度の費用内訳を円グラフで表示して」
     - 「売上総利益と営業利益の推移を分析して」

3. **コード確認・承認フロー**
   - Gemini APIが自動でPythonコードを生成
   - 生成されたコードが展開可能な形式で表示される
   - 「▶️ 実行」ボタンでコードを実行
   - 「❌ キャンセル」ボタンで実行をキャンセル可能

4. **結果確認**
   - **段階的な進捗表示**: 処理状況がステータス表示される
     - コード検証中
     - コード実行中
     - グラフ描画中
   - テキスト回答、グラフ、データテーブルが順次表示
   - グラフはPlotlyによるインタラクティブな可視化
   - データテーブルはpandasのDataFrame形式で表示

5. **会話保存**
   - サイドバーの「💾 会話を保存」ボタンでMarkdown形式で保存
   - 保存先: `notebooks/chat_logs/{セッションID}/conversation.md`
   - Obsidian等のマークダウンエディタで閲覧可能

#### 高度な機能

**参考資料アップロード**
- CSV/Excel（.xlsx）ファイルのアップロードに対応
- アップロードしたファイルはAIが自動認識して分析に活用
- タイムスタンプ付きで保存され、重複を防止
- サイドバーでアップロード済みファイルを確認可能

**グラフ自動保存**
- HTML形式（インタラクティブ、Plotly形式）
- PNG形式（静的画像、プレゼン資料用）
- 保存形式はサイドバーで選択可能（複数選択可）
- セッションディレクトリ内に自動保存

**エラー自動修正機能**
- コード実行時にエラーが発生した場合、自動で修正を試行
- 最大3回まで再試行
- エラー内容をGemini APIにフィードバックして修正コードを生成
- 修正履歴はログに記録

#### 利用可能なモデル

- **Gemini 2.5 Flash（高速・推奨）★**: 高速レスポンス、無料枠で利用可能
- **Gemini 2.5 Pro（高精度・複雑な分析）**: より高度な分析が必要な場合に選択可能

**トークン使用量の表示**
- サイドバーで現在のセッションでの総トークン数を確認可能
- APIリクエストごとにトークン数を集計
...
- **BS（貸借対照表）**: H27～R6（10年分）
  - 月次残高データ（ストックデータ）
- **科目マスタ**: PL科目マスタ、BS科目マスタ
  - 大分類・中分類での集計に対応
- **起動時データプリロード**: アプリ起動時に全データを読み込み、高速レスポンスを実現

#### 安全性機能

**多層的なセキュリティチェック**
- **コード検証**: 実行前に禁止操作を自動検出
  - ファイル削除（`os.remove`, `shutil.rmtree`等）
  - 外部プロセス起動（`subprocess`）
  - 危険な関数（`eval`, `__import__`等）
  - 不正なファイル書き込み
- **構文チェック**: Pythonの構文エラーを事前検出
- **サンドボックス実行**: 制限された環境でコードを実行
- **安全性チェック失敗時の再生成**: 最大3回まで安全なコードの生成を試行

#### システムプロンプトの強化

AIには以下の詳細情報が提供されます:
- データ構造の詳細説明（PL/BSのCSV形式、列構成）
- 利益計算方法（売上総利益、営業利益、経常利益、当期純利益）
- 科目コード体系と分類ルール
- データ読み込み関数の使用方法（キャッシュ活用）
- 実行環境で利用可能なライブラリ（pandas, plotly, numpy等）
...
- **AIチャット機能の改善**
- Gemini 2.5 Proモデルの追加（複雑な分析向け）
- トークン使用量のリアルタイム表示機能
- 段階的な進捗表示の実装（コード検証中→実行中→グラフ描画中）
- データプリロード機能の実装（起動時に全データを読み込み、高速化）
- 会話履歴の保持機能（直近5件をコンテキストとして活用）
- システムプロンプトの大幅強化
  - 利益計算方法の詳細説明（売上総利益、営業利益、経常利益、当期純利益）
  - データ読み込み関数の使用方法を明記
  - BSデータの列名変換に関する注意事項追加
  - 科目マスタとの結合方法の例示
- エラー修正時のトークン集計の改善
- セッションテーマのデフォルト値設定（「一般」）
- README.mdの大幅更新（機能説明の詳細化）
