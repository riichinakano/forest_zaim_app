...
def create_bs_monthly_trend_chart(
    ...
    Args:
        df: BS月次データ（data_loaderの出力）
        account_code: 科目コード（合算時は0）
        years: 表示する年度のリスト
        account_name: 科目名（グラフタイトル用、Noneの場合はdfから取得）
        df_master: BS科目マスタ（合算時に使用）
        category_filter: 大分類フィルタ（'流動資産'、'固定資産'等）
        subcategory_filter: 中分類フィルタ（'現金及び預金'、'有形固定資産'等）

    Returns:
        plotly.graph_objects.Figure: 月次残高推移グラフ

    Notes:
        - PLとの違い: Y軸ラベル「残高（円）」、タイトル「月次残高推移」
        - データ性質: ストック（時点残高）
    """
    # 月のリスト
    months = ['4月', '5月', '6月', '7月', '8月', '9月',
    ...
    # レイアウト設定（BS固有: タイトルとY軸ラベルを変更）
    fig.update_layout(
        title={
            'text': f"{account_name} - 月次残高推移",
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 18, 'family': 'Arial, sans-serif'}
        },
...
def create_bs_comparison_table(
    df: pd.DataFrame,
    account_code: int,
    years: List[str],
    df_master: Optional[pd.DataFrame] = None,
    ...
    if len(result_data) > 0:
        ...
        for month in months:
            if month in result_df.columns:
                avg_row[month] = result_df[month].mean()
            else:
                avg_row[month] = 0

        # 年間合計の平均
        if '年間合計' in result_df.columns:
            avg_row['年間合計'] = result_df['年間合計'].mean()
        else:
            avg_row['年間合計'] = 0
